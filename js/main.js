...
function kick(el, et) {
  ...
  // Grab the access to canvas element using getElementById() and set its dimensions
  ...
  //Now we can create a sprite object which we will call player
  //context of the canvas on which the sprite will be drawn,
  // the sprite dimensions, and the sprite sheet image.
  player = sprite({
  ...
  function finish() {
    ...
    if (endTop >= 98 && endTop <= 292 && endLeft >= 114 && endLeft <= 710) {
      if (direction == "right" && endLeft < 362) {
        ...
        incrementScore();
        if (chanceCount < 4) {
          modalElem5.setAttribute("class", "modal active");
        } else {
          if (score > 4) {
            modalElem7.setAttribute("class", "modal active");
          } else {
            modalElem6.innerHTML =
              "You scored " + score + " goal(s) out of 5. Click to try again";
        ...
        if (chanceCount < 4) {
          modalElem4.setAttribute("class", "modal active");
        } else {
          modalElem6.innerHTML =
            "You scored " + score + " goal(s) out of 5. Click to try again";
          modalElem6.setAttribute("class", "modal active");
...
var verticalIndicatorOscillate = window.setInterval(moveVerticalSmallBall, 400);
...
var horizontalIndicatorOscillate = window.setInterval(
...
var powerLevelOscillate = window.setInterval(movePowerSmallBall, 400);
...
function refreshScene() {
  ...
  document.getElementById("goal-keeper-state-2").style.display = "none"; //make left none
  document.getElementById("goal-keeper-state-3").style.display = "none"; //make right none
  document.getElementById("goal-keeper-state-1").style.display = "block"; //initial state i.e standing on middle of post.
}
...
function stopPowerBallAndKick() {
  ...
  Et =
    440 -
    ((0.8 + x1) / 1.8) * x3 * 440 +
    0.3 * x3 * (Math.abs(0.5 - x2) / 0.5) * 440;
